...
public class Board {
   ...
   private static final Color backgroundColor = new Color(32, 57, 218);
   // the color used for drawing the boundaries around the board
   private static final Color boxColor = new Color(116, 13, 217);
   ...
   // Set the board state based on the provided 2D array
   ...
   public int[][] getBoardState(){
		   ...
		   for(int row=0;row<3;row++) {
			   for(int col=0;col<3;col++) {
				   currentState[row][col]=(tiles[row][col]!=null)?tiles[row][col].getNumber():0;
   ...
   public boolean moveDown() {
      // the empty cell cannot go down if it is already at the bottom most row
      if (emptyCellRow == 2)
         return false; // return false as the empty cell cannot be moved
